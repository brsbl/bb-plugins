...
- `totals.res.discord_resolution_time`: `{ median_days, sample_size }`, measuring elapsed days from each resolved topic's earliest community feedback message in the report window to its matching commit or GitHub issue closure. Uses timestamps from normalized observations, the commit ledger, and the full issue snapshot, including issues created before the window. Each topic counts once. Matches must resolve to one known commit or issue, occur at or after the feedback and strictly before the cutoff. Predating fixes, missing timestamps, and ambiguous commit references do not enter the timing sample or change the resolution rate. The median is rounded to one decimal; it is `null` when no eligible intervals exist. Older reports may omit this field, in which case the dashboard shows an unavailable median.
