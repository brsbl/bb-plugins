...
Type `@plugin` to browse all eligible installed and Community plugins without
the usual six-results-per-group limit. Bare `@` does not return results in BB.

Installed results include running plugins with a CLI command, skill, or agent
tool. Community results include compatible plugins that are not installed.
