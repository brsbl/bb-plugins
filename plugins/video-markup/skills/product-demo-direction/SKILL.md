...
4. Register every rendered version with `video_markup_register_version`: `threadId`, `demo`, `file`, `label`, and optional `summary`. Supply `fps` only when the source has a known constant frame rate and ffprobe is unavailable. Video Markup probes actual frame timestamps when ffprobe is present, including variable frame rate footage.
5. State known rough spots before delivery. Call `video_markup_post_player` with `threadId` and `versionId`; emit its returned `::video-markup{version="…"}` directive once, on its own line, outside a code fence. Never deliver only a bare link or path. Thirteen messages in the source collaboration were about finding or playing the video.
...
| Tool | CLI | Purpose |
| --- | --- | --- |
| `video_markup_register_version` | `bb video-markup register --demo "iPhone Duo" --file /absolute/demo-v10.mp4 --label v10 --summary "Gentler camera"` | Register and carry forward unresolved feedback. |
