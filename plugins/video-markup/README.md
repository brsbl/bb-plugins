...
Attach an `.mp4`, `.webm`, or `.mov` in bb's prompt box, or ask your agent to share a video. The agent calls `video_markup_present`, emits its inline player directive, and requests the Video Markup panel for that version. Videos use their filenames and ordered numbers. The current demo is reused; the first video's filename, minus a trailing version number such as `-v1`, supplies the initial demo name. The existing demo selector switches between demos, and the panel header names the version on screen, such as **Version 1 of 3 · not the latest**.
