...
| --- | --- |
| Sidebar footer → Ambient | Scene picker and Save; each scene's sliders and four colors; Display (Visibility: how much shows through bb, Motion: speed, Detail: render resolution, Glass behind text: frosted glass behind the thread column, the composer, the sidebar's cards, and the header's pills, with a Glass opacity slider); Preview a ripple |
...
| `bb ambient` | `status`, `list`, `load`, `set <param or visibility/motion/detail/glass>=<value>`, `palette`, `save`, `delete`, `on`, `off`, `paint <description>`, `backing on\|off`, `daily` |
