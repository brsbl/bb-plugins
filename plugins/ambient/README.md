...
| --- | --- |
| Sidebar footer → Ambient | Scene picker and Save; each scene's sliders and four colors; Display (Visibility: how much shows through bb, Motion: speed, Detail: render resolution, Glass opacity: frosted glass always sits behind the thread column, the composer, the right panel, the sidebar's cards, and the header's pills so text stays readable; the slider only lowers it from its 60% default); Preview a ripple |
