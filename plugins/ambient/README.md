...
| --- | --- |
| Sidebar footer → Ambient | Scene dropdown; each scene's sliders and four colors, saved automatically as you change them, with ⌘Z / ⇧⌘Z to undo and redo. Shader edits an agent makes to the open scene save into that scene too, and Reset restores any edited scene's original version (a built-in's shipped look and the default display settings, or a saved scene as first saved). Reset and Delete each ask for a second click to confirm; Display (a light, dark, or system switch for bb's appearance, Visibility: how much shows through bb, Motion: speed, Detail: render resolution on this device only, Glass opacity: frosted glass always sits behind the thread column, the composer, the right panel, the sidebar's cards, and the header's pills so text stays readable; the slider only lowers it from its 60% default); "When an agent starts / finishes / errors" to preview how the scene reacts |
...
| `bb ambient` | `status`, `list`, `load`, `set <param or visibility/motion/glass>=<value>`, `palette`, `save`, `delete`, `on`, `off`, `reset <scene>`, `paint <description>`, `daily` |
