...
3. `action=look` returns a PNG of the raw scene. Pass `ripple: "done" | "error" | "started"` to fire a test ripple first, so you can judge how events read.
...
`color` param that mixes toward `u_canvas` so the user can fade the scene.
