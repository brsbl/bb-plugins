...
- **Theme and art:** Messenger's paper, bevel, and system-text tokens are scoped to `.bbd-im`, `.bbd-buddies`, and `.bbd-im-info` and derive from the shared palette. Light mode has pale paper; dark mode retains contrast. This is an explicit messenger type exception: Tahoma for controls and system lines, Times New Roman for conversation, Trebuchet MS for the brand, Lucida Console for code. Keep our name and original art; no AOL logo, mascot asset, advertising, or ticker.
