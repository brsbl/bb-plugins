...
| Pinball | Black cabinet, silver/red mechanisms, space-themed printed table art, purple score lettering, red ball label and silver inset side panel. Game/Options/Help rows; New Game/F2 and Pause/Resume/F3, keyboard controls in Options. Pause and clear held controls when focus leaves, the page is hidden, or the window is minimized; minimizing preserves the current game. Guide rails meet the upper flipper pivots without trapping a resting ball. Keep black readout backgrounds solid beneath dotted lettering, and keep the ball count and compact key/action control rows available in narrow windows. Artwork follows Space Cadet’s visual vocabulary but remains aligned with this plugin’s existing collision geometry; do not imply it reproduces the original mission table. |
...
- **Drag lifecycle:** Escape, blur, context menus, lost capture, pointer cancellation, resize and unmount cancel the preview. Capture, text selection and the iframe shield are always released. Native browser views hide for the gesture and return afterward. Paint keeps already drawn freehand ink (Undo removes the stroke); cancelled shape previews and canvas resizes are discarded.
