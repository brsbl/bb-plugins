...
- **Windows Media Player.** Opening it starts visualizing your microphone right away with classic visualizations (Bars, Scope, and Ambience); use the arrows or double-click the screen to switch. If it is stopped or blocked, click the screen to start. Minimize it to keep playing in the tray; close it or press stop to release the microphone. Audio never leaves the browser.
