...
- **Thread windows (bb Messenger).** Double-click a thread to open it as an early-2000s instant message window, titled "<thread> - Instant Message". bb's own chat and composer render inside. Open as many as you like; drag them by the title bar, resize them from any edge, minimize to the taskbar, or double-click the title bar to maximize. **Buddies** docks a Buddy List beside the window: every thread in the same project (or, on the Environment tab, the same environment), grouped by folder, with archived threads in a collapsed group. Working and needs-input threads sort first; idle threads are dimmed with how long they have been idle. Double-click a buddy to open it. **Get Info** opens the thread's details beside the window.
