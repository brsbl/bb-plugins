...
- **Thread windows (bb Messenger).** Double-click a thread to open it as an AOL Instant Messenger-style window, titled "<thread> - Instant Message". bb's own chat and composer render inside, with messages as AIM lines: **Me:** in red, the agent's screen name in blue. The status bar shows "<agent> is typing..." while it works. Open as many as you like; drag them by the title bar, resize them from any edge, minimize to the taskbar, or double-click the title bar to maximize. **Buddy List** docks "<project>'s Buddy List" beside the window: every thread in the same project (or, on the Environment tab, the same environment), grouped by folder with unfiled threads under **Buddies**, each group counted like AIM's (not archived/total), and archived threads in a collapsed **Offline** group. Working and needs-input threads sort first; idle threads are dimmed with how long they have been idle. Double-click a buddy to open it. **Get Info** opens the thread's details beside the window.
