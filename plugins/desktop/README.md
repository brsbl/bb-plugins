...
- **Dock.** A dock sits at the bottom of the new-thread page with one icon each for New thread, New folder, Windows Media Player, and New sticky note. A dot under an icon means its window is open; click it to bring the window forward, or click again to minimize it. Other open windows, such as folders and threads, appear after a divider. While Media Player is playing, a mini-player with the visualizer appears at the end of the dock.
...
- **On/off.** The Desktop button in the sidebar footer turns the whole desktop on or off on this device. Turning it on takes you to the new-thread page. When it is off, the new-thread page shows no desktop, windows, or dock.
