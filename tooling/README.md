...
The bootstrap prompts under [`bootstrap/`](bootstrap/) are generated from one template ([`bootstrap-prompt.template.md`](bootstrap/bootstrap-prompt.template.md)) and per-plugin parameters ([`variants.json`](bootstrap/variants.json)). Copied variants cannot silently drift: repository hygiene re-renders each one and fails on a mismatch, and it validates every automation command and permission mode against the verified interface in [`bootstrap/automation-interface.json`](bootstrap/automation-interface.json) so a generated prompt can never ship a stale command or mode. Regenerate with `node tooling/bootstrap/render-bootstrap.mjs --write`.
