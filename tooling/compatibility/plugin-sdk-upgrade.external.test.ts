...
describe("persisted 0.4.1 scaffold upgrade", () => {
  let db: DbConnection;
  ...
  it("loads a persisted scaffold and applies its tracked branch update", async () => {
    expect(PLUGIN_SDK_VERSION).toBe("0.4.1");

    const fixturePath = resolve(
      pluginRepository!,
      "tooling/compatibility/persisted-scaffold-0.4.1.json",
    );
    const fixture = JSON.parse(
      await readFile(fixturePath, "utf8"),
    ) as PersistedScaffold;
    expect(fixture.provenance).toEqual({
      repository: "https://github.com/brsbl/bb-plugins",
      commit: "755586670f40e34f46311749c97b31760905c4b0",
      sdkSourceCommit: "568715f4866dfa2070e27b750b5d6d06c040566f",
    });
    ...
    const previousCommit = await commit(sourceRepo, "persisted 0.4.1 scaffold");
    ...
    for (const [name, contents] of Object.entries(fixture.files)) {
      expect(await readFile(join(generated.directory, name), "utf8")).toBe(
        contents,
      );
    }
    expect(candidateManifest.engines).toEqual(persistedManifest.engines);
    expect(candidateManifest.devDependencies["@bb/plugin-sdk"]).toBe(
    ...
    await writeFile(
      ...
      'export default function plugin(bb: any) { bb.log.info("updated on 0.4.1"); }\n',
    );
