...
Use `npm run new:plugin -- --slug <slug> --name <name> --description <purpose>` to create a plugin, install dependencies, and run its first typecheck, test, and build. Add its screenshot and root README entry with the [plugin catalog entry template](tooling/plugin-catalog-entry.md) before opening a PR.
